...
| **Nepal Admin** | Wilson, Anmol, Anusha, Monika, Sunam Deepa | `dashboard`, `tasks`, `attendance`, `production`, `qc`, `inventory`, `finance`, `billing`, `content` (Budget), `employees`, `customers`, `messenger`, `library` | All default Nepal Operations (Wilson, Anmol, & Anusha have forced `production` write access; Anusha has forced `tasks` write/edit access; Sunam Deepa manages Finance & Accounting) |
